...
export const loginUserHandleController = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const getUserController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = await getInfoUserService(res.locals.userData.id);
...
export const updateInfoUserController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = await updateInfoUserService(res.locals.userData.id, req.body);
    return res.status(StatusCodes.OK).json(user);
  } catch (err) {
    return next(err);
  }
