...
export const getTheBookService = async (optionQuery: IOftionQueryBook) => {
  try {
    const typeOfTheBook = {
    ...
    const options = {
      ...optionQuery,
      sort: { [optionQuery.sort]: sortType },
      populate: { path: "author", select: "-password -__v -updatedAt -createdAt" },
    };
    const listOfTheBook = await Book.paginate(optionQuery.typeOfBook ? typeOfTheBook : {}, options);
...
export const listMemberSubscribeBookService = async (idBook: string, optionQuery: IOftionQueryBook) => {
  try {
    ...
    const listOfTheBook = await Book.paginate(optionQuery.typeOfBook ? queryFilter : { _id: idBook }, options);
